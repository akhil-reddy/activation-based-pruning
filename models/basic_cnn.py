...
class PrunableNeuralModel(nn.Module):
    def __init__(self, layer_data):
        ...
        for type, dims in layer_data.items():
            if(type == "conv"):
                self.layers.append(nn.Conv2d(layer_dims[i - 1], layer_dims[i], bias=False))
            if(type == "pool"):
                self.layers.append(nn.MaxPool2d(layer_dims[i - 1], layer_dims[i], bias=False))
            if(type == "fc"):
